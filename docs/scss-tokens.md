...
Three blind spots produce *apparent* problems that aren't real, and the audit accounts for them:
...
- `--tool-panel-bar-background`, `--tool-panel-bar-foreground`, `--tool-panel-body-background` —
  ...
  override). The scan didn't parse TS-embedded CSS *definitions*.
...
presence), then diff the two sets. Remember to exclude the three blind spots in *Method & caveats*
