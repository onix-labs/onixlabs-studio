...
class FakeTransport implements DockerTransport {
  ...
  public readonly streams: { onLine: (line: string) => void; onError: (error: Error) => void; closed: boolean }[] =
    [];
  public responder: (method: string, path: string) => Promise<DockerResponse> = (): Promise<DockerResponse> =>
  ...
  public openStream(
    ...
    const record: { onLine: (line: string) => void; onError: (error: Error) => void; closed: boolean } = {
      onLine,
      onError,
      closed: false,
    };
 ...
 */
const SOCKET: DockerSocket = { path: '/unused.sock', connect: (): Promise<never> => Promise.reject(new Error('unused')) };
 ...
 */
function respondJson(path: string, body: unknown): (method: string, path: string) => Promise<DockerResponse> {
...
describe('DockerEngine', () => {
  ...
  it('listImages_mapsTheRawEngineShape', async () => {
    ...
    expect(await engine.listImages()).toEqual([{ id: 'sha256:1', tags: ['nginx:latest'], size: 1234 }]);
  });
  ...
  it('listImages_returnsEmptyOnANon2xxStatus', async () => {
    const { engine, transport } = engineWith();
    transport.responder = (): Promise<DockerResponse> => Promise.resolve({ statusCode: 500, body: 'nope' });
  ...
  it('start_stop_remove_issueTheRightRequestsAndReportSuccess', async () => {
    const { engine, transport } = engineWith();
    transport.responder = (): Promise<DockerResponse> => Promise.resolve({ statusCode: 204, body: '' });
  ...
  it('watch_pushesNormalisedEventsAndIgnoresMalformedLines', () => {
    ...
    transport.streams[0].onLine(JSON.stringify({ Type: 'container', Action: 'start', Actor: { ID: 'abc' } }));
...
describe('parseEvent', () => {
  it('defaultsMissingFieldsAndReadsTheActorId', () => {
    expect(parseEvent(JSON.stringify({ Type: 'image' }))).toEqual({ type: 'image', action: '', id: '' });
  });
