...
import { ContributionOrigin, GrantDecision, PermissionDeniedError, PermissionId } from './permission';
 ...
 */
function fakeFactories(): { factories: ReadonlyMap<PermissionId, PermissionFactory>; create: Mock } {
  const create: Mock = vi.fn((): { readonly kind: string } => HANDLE);
  const factory: PermissionFactory = { id: 'docker.socket', create };
  return { factories: new Map<PermissionId, PermissionFactory>([['docker.socket', factory]]), create };
}
 ...
 */
function request(declared: PermissionId[], origin: ContributionOrigin = 'first-party'): PermissionRequest {
