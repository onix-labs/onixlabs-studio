 ...
 *
 * A fence takes its Monaco from the window it is in. Monaco settles focus against the document it was
 * loaded into and cannot be told about another, so an editor built in a modal with the main window's
 * instance is forever unfocused there — caret hidden, selection painted inactive, Home/End/undo dead.
 * The {@link Monaco} service loads an instance per window, and every fence asks for the one that
 * belongs to its own document.
 */
...
class MonacoCodeBlockView implements NodeView {
  ...
  /**
   * Holds the Monaco instance of the window this fence is in, from its first mount on; null until a
   * mount has resolved it. Every editor, model and range of this fence comes from this instance.
   */
  private monaco: typeof MonacoApi | null = null;
  ...
  public update(node: ProseMirrorNode): boolean {
    ...
    const monaco: typeof MonacoApi | null = this.monaco;
    if (languageChanged) {
      monaco?.editor.setModelLanguage(model, this.resolvedLanguageId());
    }
  ...
  public setSelection(anchor: number, head: number): void {
    void this.mountEditor(false).then((): void => {
      ...
      if (editor === null || monaco === null || model === null) {
        return;
      }
      editor.focus();
      this.applyingRemoteEdit = true;
  ...
  private async mountEditor(focus: boolean): Promise<void> {
    ...
    this.mounting = true;
    let monaco: typeof MonacoApi | null;
    try {
      // The instance for this fence's own window: the main window's, or one loaded into the child
      // window a modal presents the editor in.
      monaco = await this.deps.monaco.ensureLoadedIn(this.dom.ownerDocument);
    } catch {
      monaco = null;
    } finally {
      this.mounting = false;
    ...
    if (monaco === null || this.editor !== null || !this.dom.isConnected) {
      return;
    }
    this.monaco = monaco;
  ...
  private forwardEdit(event: MonacoApi.editor.IModelContentChangedEvent): void {
    ...
    // Focus is judged by containment of the fence's own document's active element, which covers the
    // editor and the language field alike and holds in whichever window the fence is in.
  ...
  private scheduleTeardown(): void {
    ...
    this.teardownTimer = setTimeout((): void => {
      ...
      // language field, in whichever window the fence is in.
