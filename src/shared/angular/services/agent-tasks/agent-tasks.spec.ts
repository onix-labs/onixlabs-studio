...
function owner(
  ...
  tasks: WritableSignal<readonly AgentTask[]>;
  revealed: string[];
  stopped: string[];
} {
...
describe('AgentTasks', () => {
  ...
  it('tasks_areAggregatedAcrossConversations_andAttributedToTheirAgent', () => {
    ...
    const a = owner('Refactor', [task('t1')], 'tab-1');
    const b = owner('Docs', [task('t2'), task('t3')], 'tab-2');
  ...
  it('unregister_dropsTheConversation_soAClosedTabCannotBeNamed', () => {
    const registry: AgentTasks = make();
    const a = owner('Refactor', [task('t1')]);
  ...
  it('unregister_afterTheIdWasReRegistered_leavesTheLiveConversationAlone', () => {
    const registry: AgentTasks = make();
    const first = owner('First', [task('t1')]);
    const second = owner('Second', [task('t2')]);
  ...
  it('revealAndStop_areRoutedToTheOwningConversation', () => {
    const registry: AgentTasks = make();
    const a = owner('Refactor', [task('t1')]);
    const b = owner('Docs', [task('t2')]);
    registry.register('conv-a', a.owner);
    registry.register('conv-b', b.owner);

    const target: LiveAgentTask | undefined = registry
      .tasks()
      .find((t: LiveAgentTask): boolean => t.taskId === 't2');
    expect(target).toBeDefined();
    registry.reveal(target as LiveAgentTask);
    registry.stop(target as LiveAgentTask);
