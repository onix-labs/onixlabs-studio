...
import { MarkdownRenderer, resetTrustedBlockCache } from './markdown-renderer';

@Component({
  imports: [MarkdownRenderer],
  template: `<app-markdown-renderer [text]="text()" />`,
})
class TestHost {
...
describe('MarkdownRenderer', () => {
  ...
  it('renders prose as HTML', () => {
    ...
    const prose: HTMLElement | null = host.querySelector('.markdown-renderer__prose');
  ...
  it('keeps prose and code as separate ordered blocks', () => {
    ...
    fixture.detectChanges();
    expect(host.querySelectorAll('.markdown-renderer__prose').length).toBe(2);
  ...
  it('renders inline math through KaTeX', () => {
    ...
    fixture.detectChanges();
    expect(host.querySelector('.markdown-renderer__prose .katex')).not.toBeNull();
  });
  ...
  it('keeps a settled block’s DOM node while a later block streams', () => {
    ...
    fixture.detectChanges();
    const settled: HTMLElement | null = host.querySelector('.markdown-renderer__prose');
    expect(settled).not.toBeNull();
    component.text.set('First paragraph.\n\nSecond is growing longer');
    fixture.detectChanges();
    expect(host.querySelector('.markdown-renderer__prose')).toBe(settled);
  });

  it('renders nothing for empty text', () => {
    component.text.set('');
    fixture.detectChanges();
    expect(host.querySelector('.markdown-renderer__prose')).toBeNull();
