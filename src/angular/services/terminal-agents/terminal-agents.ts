...
/**
 * Holds the panel state for a single terminal tab's docked agent.
 */
...
/**
 * Holds the panel state assumed for a terminal tab that has no entry yet.
 */
...
/**
 * Owns the docked agent-panel state for terminal tabs: whether each tab's agent panel is shown. The
 * agent always docks to the side, so there is no layout to track; it can be open at the same time as
 * the terminal.
 *
 ...
 */
@Service()
export class TerminalAgents {
  /**
   * Holds the panel state for every terminal tab, keyed by tab identifier.
   */
