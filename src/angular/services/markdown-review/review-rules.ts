...
function clip(value: string, fromEnd: boolean): string {
  ...
  return fromEnd ? `…${value.slice(value.length - CONTEXT_LEN)}` : `${value.slice(ZERO, CONTEXT_LEN)}…`;
}
...
function findRuleIssues(source: string, masked: string): ReviewIssue[] {
  ...
  for (const rule of RULES) {
    ...
    while ((match = rule.regex.exec(masked)) !== null) {
      ...
      issues.push(makeIssue(source, built.start, built.end, rule.kind, rule.name, built.suggestions));
    }
  }
  return issues;
}
